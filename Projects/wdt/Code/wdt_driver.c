...
static void wait_for_wdt1(wdt_index_t wdt_ind)
{
	if(wdt_ind == WDT1)
	{
		while (!REG_GET_BIT(WDTCTL(WDT1),BIT(WRC_IDX)));
	}
}
...
int32_t wdt_init(wdt_cfg_t* pwdt_cfg)
{
	...
	if(pwdt_cfg != NULL)
	{
		REG_SET_BIT(RCGCWD,BIT(pwdt_cfg->wdt_index));
		...
		{
			REG_SET_BIT(WDTCTL(pwdt_cfg->wdt_index),BIT(RESEN_IDX));
		}
		wait_for_wdt1(pwdt_cfg->wdt_index);
		if (pwdt_cfg->wdt_int == USE_NMI)
		{
			REG_SET_BIT(WDTCTL(pwdt_cfg->wdt_index),BIT(INTTYPE_IDX));
			wait_for_wdt1(pwdt_cfg->wdt_index);
			REG_SET_BIT(WDTCTL(pwdt_cfg->wdt_index),BIT(INTEN_IDX));
      wait_for_wdt1(pwdt_cfg->wdt_index);
		...
		{
			REG_SET_BIT(WDTTEST(pwdt_cfg->wdt_index),BIT(STALL_IDX));
			wait_for_wdt1(pwdt_cfg->wdt_index);
		}
...
{
	bool result = FALSE;
	if(REG_GET_BIT(WDTRIS(wdtn),BIT(WDTRIS_IDX)))
	{
		result = TRUE;
	}
	return result;
